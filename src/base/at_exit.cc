...
void AtExitManager::RegisterTask(base::OnceClosure task) {
  std::cout <<"mod:AtExitManager::RegisterTask:check point 1:" << g_top_manager << std::endl;
  if (!g_top_manager) {

    /*skipped for avoiding nullptr exception: logging::LogMessage::~LogMessage()
    NOTREACHED() << "Tried to RegisterCallback without an AtExitManager";
    */
    return;
  }
  std::cout <<"mod:AtExitManager::RegisterTask:check point 2" << std::endl;
  AutoLock lock(g_top_manager->lock_);
#if DCHECK_IS_ON()
  std::cout <<"mod:AtExitManager::RegisterTask:check point 3" << std::endl;
  DCHECK(!g_top_manager->processing_callbacks_);
  std::cout <<"mod:AtExitManager::RegisterTask:check point 4" << std::endl;
#endif
