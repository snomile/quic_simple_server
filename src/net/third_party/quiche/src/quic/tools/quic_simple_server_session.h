...
class QuicSimpleServerSession : public QuicServerSessionBase {
 public:
  ...
  void OnCanCreateNewOutgoingStream(bool unidirectional) override;
    ...
    //add netlog for headers
  void OnHeaderList(const QuicHeaderList& header_list) override;
 ...
 private:
  ...
  //add netlog in quic_connection
  void setConnectionLogger();  
  std::unique_ptr<net::FileNetLogObserver> net_log_file_observer_;
  net::NetLogWithSource netlogWithSource;
  net::QuicConnectionLogger* logger_;

  //add netlog for handshake message in session
  void OnCryptoHandshakeMessageSent(
      const quic::CryptoHandshakeMessage& message) override;
  void OnCryptoHandshakeMessageReceived(
      const quic::CryptoHandshakeMessage& message) override;

  base::FilePath getNetLogName();
